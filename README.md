...
Update: Consider fitting the updated BAA, Long rates, with December average. Data file is data.xlsx and we have three code files: 2d.py fits the updated (BAA, Long) data. We failed to make innovations Gaussian IID. log-rates.py is the code fitting the updated log(BAA), log(Long) models. We were able to fit only log(BAA-Long) as a simple autoregression, without volatility. risk-spread.py is the code fitting total normalized returns of S&P 500 vs spreads, and we can fit only the model vs spreads/vol, with or without logarithm. See the blog post https://my-finance.org/2026/05/25/including-bond-factors-into-the-new-gaussian-model/
