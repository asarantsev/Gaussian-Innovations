...
# T = time horizon
def sim(initialV, initialR, initialM, initialL, T):
    ...
    simLSpreads = numpy.log(numpy.log(initialR) - numpy.log(initialL)) * numpy.ones(NSIMS)
    ...
    simLong = np.exp(simLRates - np.exp(simLSpreads))
    simSpreads = simRates - simLong # and simulated spreads
    ...
    for t in range(T):
        ...
        simRetIntl[t] = numpy.exp(0.2111 * numpy.ones(NSIMS) - 0.0196 * simVol[t+1] - 0.0316 * (simRates[t+1] - simRates[t]) - 0.0641 * simMeasure[t] + 0.0387 * simSpreads[t] + simVol[t+1] * noiseIntl[t]) - numpy.ones(NSIMS)
