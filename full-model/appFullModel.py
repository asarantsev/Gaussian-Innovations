...
# given that we already have simulated five asset classes
...
# trShare = share of USA Treasuries in the bond part of the portfolio
# the rest are corporate USA bonds
# intlShare = share of ex-USA in the stock part of the portfolio
# the rest are USA stocks measured by the S&P 
# emShare = share of emerging stocks in the international part of the stock portfolio
# the rest are ex-USA developed markets stocks
...
def simWealth(simReturns, initialW, initialFlow, growthFlow, nYears, bondStart, bondEnd, intlShare, emShare, trShare):
    ...
    simRetUsaStocks = simReturns[0]
    simRetDevStocks = simReturns[1]
    simRetEmStocks = simReturns[2]
    simRetTrBonds = simReturns[3]
    simRetCorpBonds = simReturns[4]
        
    # simulate the international stock part of the portfolio
    simRetIntl = simRetDevStocks * (1 - emShare) + simRetEmStocks * emShare
    
    # simulate the overall stock part of the portfolio
    simRetStock = simRetIntl * intlShare + simRetUsaStocks * (1 - intlShare)
    
    # simulate the bond part of the portfolio
    simRetBond = simRetTrBonds * trShare + simRetCorpBonds * (1 - trShare)
    ...
    for t in range(nYears):
        simPortfolio[t] = bondP[t] * simRetBond[t] + stockP[t] * simRetStock[t]
...
def setupText(initialWealth, initialFlow, growthFlow, timeHorizon, bondShare0, bondShare1, intlShare, emShare, trShare, initV, initH, initR, initL):
    ...
    # this part of text output explains flows (withdrawals or contributions)
    # which can be constant or increase annually at constant rate
    flowText = initialFlowText + ' ' + growthText

    # text output explaining initial conditions
    initCondText = '\nINITIAL CONDITIONS for MARKET FACTORS'
    initVText = 'initial S&P annual volatility: ' + str(initV)
    initHText = 'initial S&P valuation measure: ' + str(initH)
    initRText = 'initial Moody BAA rate: ' + str(initR)
    initLText = 'initial 10-year Treasury rate: ' + str(initL)
    ...
    texts = [simText, stockText, initText, devText, bondText, startText, endText, timeText, initWealthText, flowText, initCondText, initVText, initHText, initRText, initLText]
...
def output(simReturns, initialW, initialFlow, growthFlow, timeHorizon, bondStart, bondEnd, intlShare, emShare, trShare, initV, initH, initR, initL, fileName):

    pathData, paths = simWealth(simReturns, initialW, initialFlow, growthFlow, timeHorizon, bondStart, bondEnd, intlShare, emShare, trShare)
    ...
    SetupText = setupText(initialW, initialFlow, growthFlow, timeHorizon, bondStart, bondEnd, intlShare, emShare, trShare, initV, initH, initR, initL)
    ...
    plt.legend(bbox_to_anchor = (1, 1.1), loc = 'upper left', prop = {'size': 10}, facecolor = 'azure')
